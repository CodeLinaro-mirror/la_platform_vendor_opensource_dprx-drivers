...
#define VIRTIO_MEDIA_PROPERTY_BLOB 0x1000 
/**
 * struct virtio_media_cmd_propertyblob - Driver command for VIRTIO_MEDIA_CMD_PROPBLOB.
 * @hdr: header with cmd member set to VIRTIO_MEDIA_CMD_PROPBLOB.
 * @session_id: id of the session to run the ioctl on.
 * @property_id : property ro be read or write.
 * @write_or_read: read ot write property.
 * @data: space to read or data to write.
 */
struct virtio_media_cmd_propblob {
    struct virtio_media_cmd_header hdr;
    u32 session_id;
    u32 property_id;
    u32 write_or_read;
...
/**
 * struct virtio_media_resp_property_blob - Device response for VIRTIO_MEDIA_CMD_PROPBLOB.
 * @hdr: header containing the status of the property.
 */
struct virtio_media_resp_property_blob {
    struct virtio_media_resp_header hdr;
    u32 property_id;
    u32 data[8];
};
